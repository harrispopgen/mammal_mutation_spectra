...
# will be quite slow
flagOfAnalysis=paste0(todaysdate,".plots.SubsampledIndividualsNotProjection.epsilon.",epsilon,".sepCpGs.EXCLUDED.masked",maskLabel,".RESCALEDTOHUMANTARGETS.SimplifiedCode.",as.character(mantelTestPermutationCount),".MantelPermutations.CpG.TpGEXCLUDED") # a label that will be in title of plots and outdir 
...
############# EXCLUDE CPG.TPG MUTATIONS FROM SPECTRUM ##############
all1merSpectra_HumanRescaled_noCpGTpG <- all1merSpectra_HumanRescaled %>%
  filter(mutation_1mer!="CpG.TpG")
...
##### ExCLUDING CPGS!!! 
all1merSpectra <- downsampleFunc_RescaledByHumanTargets(all1merSpectra_HumanRescaled_noCpGTpG,plotdir)

if("CpG.TpG" %in% all1merSpectra$mutation_1mer){
  print("you didn't exclude cpgs properly!")
}

# write these out:
write.table(all1merSpectra,paste0(plotdir,"all1merSpectra.plusCpGs.CountsAndTargets.RESCALED_BY_HUMAN_TARGETS.DOWNSAMPLED.ExcludedCpGTpG.txt"),quote=F,sep="\t",row.names = F)
...
# then write this out.
write.table(all_processed_spectra_to_write_out,paste0(plotdir,'AllProcessedSpectra.AllConditions.AllVariables.AllSpecies.RESCALED_BY_HUMAN_TARGETS.DOWNSAMPLED.PROCESSED.CpGsExcluded.txt'),quote=F,sep="\t",row.names = F)
...
write.table(all_distances_all_conditions_phylo,paste0(plotdir,"all_distances_all_conditions_phylo.1merNoCpGTpg.txt"),row.names = F,quote=F,sep = "\t") 

# write out mantel results
write.table(data.frame(mantel_test_results_raxml_SQRTDISTANCE_df),paste0(plotdir,"mantel_test_results_raxml_SQRTDISTANCE_df.txt"),row.names = F,quote=T,sep = "\t") # for some reason needed quote =t
