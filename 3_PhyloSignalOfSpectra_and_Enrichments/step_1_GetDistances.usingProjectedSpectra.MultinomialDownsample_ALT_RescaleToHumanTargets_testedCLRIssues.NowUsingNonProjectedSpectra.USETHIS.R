...
# will be quite slow
flagOfAnalysis=paste0(todaysdate,".plots.SubsampledIndividualsNotProjection.epsilon.",epsilon,".sepCpGs.no.masked",maskLabel,".RESCALEDTOHUMANTARGETS.SimplifiedCode.",as.character(mantelTestPermutationCount),".MantelPermutations") # a label that will be in title of plots and outdir 
...
confounders <- read.table("/Users/annabelbeichman/Documents/UW/multispecies_spectra_comparisons/information/confoundersToPlotPhyloSignal.GenomeN50.etc.updatedthetavals.updatedCoverage.txt",sep="\t",header=T) # updated values of theta and coverage to be based on teh 5 inds that were picked at random per pop
head(confounders)
...
write.table(phyloDistances,paste0(plotdir,"pairwise.phylogeneticDistance.raxmltree.txt"),row.names=F,col.names = T,sep="\t") # for use in sigfit plots
...
##### 1mers are dealt with in the if statement below this
# get ancestral kmers:
...
# want to get ancestral 1mers for all data frames so I can plot things with central mutation type separated later:
...
# you are ehre
########## write out nice versions for manuscript ############
# restrict to just the chosen populations
colsToWriteForMS=c("species","population","label","mutation_label","total_mutations","total_target_count")



write.table(all1merSpectra_plusTargets[all1merSpectra_plusTargets$label %in% speciesToInclude_phylo,colsToWriteForMS],paste0(plotdir,"FORMANUSCRIPT.all1merSpectra_plusTargets.subsetPops.notHumanRescaled.NotDownsampled.txt"),row.names=F,quote=F,sep="\t")

write.table(all3merSpectra_plusTargets[all3merSpectra_plusTargets$label %in% speciesToInclude_phylo,colsToWriteForMS],paste0(plotdir,"FORMANUSCRIPT.all3merSpectra_plusTargets.subsetPops.notHumanRescaled.NotDownsampled.txt"),row.names=F,quote=F,sep="\t")

write.table(all5merSpectra_plusTargets[all5merSpectra_plusTargets$label %in% speciesToInclude_phylo,colsToWriteForMS],paste0(plotdir,"FORMANUSCRIPT.all5merSpectra_plusTargets.subsetPops.notHumanRescaled.NotDownsampled.txt"),row.names=F,quote=F,sep="\t")

write.table(all7merSpectra_plusTargets[all7merSpectra_plusTargets$label %in% speciesToInclude_phylo,colsToWriteForMS],paste0(plotdir,"FORMANUSCRIPT.all7merSpectra_plusTargets.subsetPops.notHumanRescaled.NotDownsampled.txt"),row.names=F,quote=F,sep="\t")
...
# FOR MANUSCRIPT: write out a more organized version of this:
colsToWriteForMS2 <- c("species","population","label","mutation_label","total_mutations_ORIGINAL","total_mutations_multinom_downsampled_RESCALED_BY_HUMAN_TARGETS")

write.table(all1merSpectra[all1merSpectra$label %in% speciesToInclude_phylo,colsToWriteForMS2],paste0(plotdir,"FORMANUSCRIPT.all1merSpectra.Counts.RESCALED_BY_HUMAN_TARGETS.THEN.DOWNSAMPLED.txt"),quote=F,sep="\t",row.names = F)
write.table(all3merSpectra[all3merSpectra$label %in% speciesToInclude_phylo,colsToWriteForMS2],paste0(plotdir,"FORMANUSCRIPT.all3merSpectra.Counts.RESCALED_BY_HUMAN_TARGETS.THEN.DOWNSAMPLED.txt"),quote=F,sep="\t",row.names = F)
write.table(all5merSpectra[all5merSpectra$label %in% speciesToInclude_phylo,colsToWriteForMS2],paste0(plotdir,"FORMANUSCRIPT.all5merSpectra.Counts.RESCALED_BY_HUMAN_TARGETS.THEN.DOWNSAMPLED.txt"),quote=F,sep="\t",row.names = F)
write.table(all7merSpectra[all7merSpectra$label %in% speciesToInclude_phylo,colsToWriteForMS2],paste0(plotdir,"FORMANUSCRIPT.all7merSpectra.Counts.RESCALED_BY_HUMAN_TARGETS.THEN.DOWNSAMPLED.txt"),quote=F,sep="\t",row.names = F)
...
# write out just a version with the variable used for mantel test and CLR transform:
colsToWriteForMS3=c("id","mutation_label","variable","transform_id",speciesToInclude_phylo)

for(id in c("1-mer spectrum","3-mer spectrum","5-mer spectrum","7-mer spectrum")){
write.table(all_processed_spectra_to_write_out[all_processed_spectra_to_write_out$variable==variableToUseInMantelTest & all_processed_spectra_to_write_out$transform_id=="clr" & all_processed_spectra_to_write_out$id==id,colsToWriteForMS3],paste0(plotdir,'FORMANUSCRIPT.',id,'.',variableToUseInMantelTest,'.subsetPops.RESCALED_BY_HUMAN_TARGETS.DOWNSAMPLED.PROCESSED.CLRTRANSFORM.wide.txt'),quote=F,sep="\t",row.names = F)
}
...
# get distances for plotting (don't need to do for 10k fake spectra): 
...
ggsave(paste0(controldatadir,"manteltest.sqrtPhylo.exploded5merAnalyses.ComparingFakeEmpirical.FakeGeneratedByMultinomSamplOverTargetSizesWithin3mer.pdf"),mantelTestCorrelationsPlot_phylo_SQRTDISTANCE_BasedOnFAKE5merSpectrum,height=5,width=9)
 ############### array of fake 5mer spectra ##############
# for the ARRAY of fake spectra:

# want to replicate this 5K times -- these are now 5,000 fake spectra 
FAKE_all5merSpectra_humanRescaled_ARRAY=list()
numFakeSpectraPermutationsToDo=5000
for(i in seq(1,numFakeSpectraPermutationsToDo)){
  
  FAKE_all5merSpectra_humanRescaled_ARRAY[[i]] <- makeFakeControl5merSpectrumFunction_humanRescaled(all5merSpectra_HumanRescaled) 
  
}

### downsample the array
dir.create(paste0(controldatadir,"/array/"))
FAKE_all5merSpectra_humanRescaled_ARRAY_DOWNSAMPLED <- lapply(FAKE_all5merSpectra_humanRescaled_ARRAY,downsampleFunc_RescaledByHumanTargets,paste0(controldatadir,"/array/"))

# want to run on fake 5mer array, not including full mantel results and noly using 1 permutations ecause just want cor (this is lazy coding for now, should really just write code to calculate )
mantel_pearsonROnly_Fake5mer_ARRAY <- lapply(FAKE_all5merSpectra_humanRescaled_ARRAY_DOWNSAMPLED,comboFunction_mantelTest_SQRTDISTANCE,phylo_dist= raxml_cophenetic_dist,variable=variableToUseInMantelTest,speciesToInclude_phylo,mantelTestPermutationCount=1,epsilon=epsilon,speciesCodes=speciesCodes) # just using 1 mantel perm because I really just want the cor value (lazy coding)

mantel_pearsonROnly_Fake5mer_ARRAY_df <- bind_rows(mantel_pearsonROnly_Fake5mer_ARRAY,.id="id")

# p value is num of values greater than or equal to empirical (+1 (empirical)) / (total perms +1 (empirical))
pValOnFake5merArrayRValuesGreaterThanEmpirical <- (sum(mantel_pearsonROnly_Fake5mer_ARRAY_df$statistic >= mantel_test_results_raxml_SQRTDISTANCE_df[mantel_test_results_raxml_SQRTDISTANCE_df$id=="5-mer spectrum",]$statistic)+1)/(numFakeSpectraPermutationsToDo+1)

pValOnFake5merArrayRValuesGreaterThanEmpirical
# want to look at dist of correlations and then the real correlation
Fake5merArrayPlotComparedToEmpirical<-ggplot(mantel_pearsonROnly_Fake5mer_ARRAY_df,aes(x=statistic))+
  geom_histogram()+
  geom_vline(data=mantel_test_results_raxml_SQRTDISTANCE_df[mantel_test_results_raxml_SQRTDISTANCE_df$id=="5-mer spectrum",],aes(xintercept=statistic,color="empirical r value"),size=2)+
  ggtitle(paste0("array of ",numFakeSpectraPermutationsToDo,"\npermuted-within-3mer 5-mer spectra"))+
  annotate("text",x=0.7,y=450,label=paste0("p-value: ",round(pValOnFake5merArrayRValuesGreaterThanEmpirical,5)))+
  theme_bw()  +
  xlab("Pearson's r\n(correlation between permuted spectra\nand sqrt phylogenetic distance)")+
  ylab("count")+
  theme(legend.position = "none",)
Fake5merArrayPlotComparedToEmpirical

ggsave(paste0(controldatadir,"manteltest.sqrtPhylo.exploded5merAnalyses.ComparingFakeEmpirical.FakeGeneratedByMultinomSamplOverTargetSizesWithin3mer.ARRAYofRValues.pdf"),Fake5merArrayPlotComparedToEmpirical,height=5,width=6)

write.table(mantel_pearsonROnly_Fake5mer_ARRAY_df,paste0(controldatadir,"manteltest.sqrtPhylo.exploded5merAnalyses.ComparingFakeEmpirical.FakeGeneratedByMultinomSamplOverTargetSizesWithin2mer.ARRAYofRValues.RESULTS.txt"),quote=F,row.names=F,sep="\t")

# get rid of this from memory since it's huge:
rm(FAKE_all5merSpectra_humanRescaled_ARRAY)
...
############## array of fake 7mer spectra #########

# want to replicate this 5K times -- these are now 5,000 fake spectra 
mantel_pearsonROnly_Fake7mer_ARRAY_df=data.frame()
for(i in seq(1,numFakeSpectraPermutationsToDo)){
  print(i)
  # generate fake spectrum:
  FAKE_all7merSpectra_humanRescaled_Iteration <- makeFakeControl7merSpectrumFunction_humanRescaled(all7merSpectra_HumanRescaled) # 7mer spectra
  
  # downsample it: 
  FAKE_all7merSpectra_humanRescaled_Iteration_Downsample <- downsampleFunc_RescaledByHumanTargets(FAKE_all7merSpectra_humanRescaled,paste0(controldatadir,"/array/"))
  
  # run mantel with 1 perm (just want the correlation value)
  FAKE_all7merSpectra_mantelTestResults_Iteration <- comboFunction_mantelTest_SQRTDISTANCE(FAKE_all7merSpectra_humanRescaled_Iteration_Downsample,phylo_dist= raxml_cophenetic_dist,variable=variableToUseInMantelTest,speciesToInclude_phylo,mantelTestPermutationCount=1,epsilon=epsilon,speciesCodes=speciesCodes)
  
  FAKE_all7merSpectra_mantelTestResults_Iteration$iteration <- i
  
  mantel_pearsonROnly_Fake7mer_ARRAY_df <- bind_rows(mantel_pearsonROnly_Fake7mer_ARRAY_df,FAKE_all7merSpectra_mantelTestResults_Iteration)
  
  # don't need to rm from mem because are being overwritten each loop
}

# (num greater than or EQUAL To empirical +1) / (total perm +1)
pValOnFake7merArrayRValuesGreaterThanEmpirical <- (sum(mantel_pearsonROnly_Fake7mer_ARRAY_df$statistic>=mantel_test_results_raxml_SQRTDISTANCE_df[mantel_test_results_raxml_SQRTDISTANCE_df$id=="7-mer spectrum",]$statistic)+1)/(numFakeSpectraPermutationsToDo+1)

pValOnFake7merArrayRValuesGreaterThanEmpirical
# want to look at dist of correlations and then the real correlation
Fake7merArrayPlotComparedToEmpirical<-ggplot(mantel_pearsonROnly_Fake7mer_ARRAY_df,aes(x=statistic))+
  geom_histogram()+
  geom_vline(data=mantel_test_results_raxml_SQRTDISTANCE_df[mantel_test_results_raxml_SQRTDISTANCE_df$id=="7-mer spectrum",],aes(xintercept=statistic,color="empirical r value"),size=2)+
  ggtitle(paste0("array of ",dim(mantel_pearsonROnly_Fake7mer_ARRAY_df)[1],"\npermuted-within-5mer 7-mer spectra"))+
  annotate("text",x=0.52,y=400,label=paste0("p-value: ",round(pValOnFake7merArrayRValuesGreaterThanEmpirical,3)))+
  theme_bw()+
  xlab("Pearson's r\n(correlation between permuted spectra\nand sqrt phylogenetic distance)")+
  ylab("count")+
  theme(legend.position="none")
Fake7merArrayPlotComparedToEmpirical

ggsave(paste0(controldatadir,"manteltest.sqrtPhylo.exploded7merAnalyses.ComparingFakeEmpirical.FakeGeneratedByMultinomSamplOverTargetSizesWithin5mer.ARRAYofRValues.pdf"),Fake7merArrayPlotComparedToEmpirical,height=5,width=6)


write.table(mantel_pearsonROnly_Fake7mer_ARRAY_df,paste0(controldatadir,"manteltest.sqrtPhylo.exploded7merAnalyses.ComparingFakeEmpirical.FakeGeneratedByMultinomSamplOverTargetSizesWithin5mer.ARRAYofRValues.RESULTS.txt"),quote=F,row.names=F,sep="\t")

################ plot exploded r distribution plots side by side for manuscript ###########
fake5mer7merdistributionPlotsArrange <- ggarrange(Fake5merArrayPlotComparedToEmpirical,Fake7merArrayPlotComparedToEmpirical)
ggsave(paste0(controldatadir,"manteltest.sqrtPhylo.explodedkmerAnalyses.ARRAYofRValues.5mer-7merCombo.pdf"),fake5mer7merdistributionPlotsArrange,height=4,width=8)
...
ylimmin_rValues_13mer_nonsep=min(all_confounder_spectrum_mantel_results_PLUSphylodist[all_confounder_spectrum_mantel_results_PLUSphylodist$niceLabelForPlotting!="phyloMantel" & all_confounder_spectrum_mantel_results_PLUSphylodist$id %in% c("1-mer spectrum","3-mer spectrum"),]$statistic)

ylimmax_rValues_13mer_nonsep=max(all_confounder_spectrum_mantel_results_PLUSphylodist[all_confounder_spectrum_mantel_results_PLUSphylodist$niceLabelForPlotting!="phyloMantel" & all_confounder_spectrum_mantel_results_PLUSphylodist$id %in% c("1-mer spectrum","3-mer spectrum"),]$statistic)
# add 20% so that annotations will show up
ylimmax_rValues_13mer_nonsep=ylimmax_rValues_13mer_nonsep+.2*ylimmax_rValues_13mer_nonsep

allConfounderMantelResultsPlot_rValues_NOPHYLOMANTEL_13meronly <- ggplot(data.frame(all_confounder_spectrum_mantel_results_PLUSphylodist[all_confounder_spectrum_mantel_results_PLUSphylodist$niceLabelForPlotting!="phyloMantel" & all_confounder_spectrum_mantel_results_PLUSphylodist$id %in% c("1-mer spectrum","3-mer spectrum"),]),aes(x=niceConfounderLabel,y=statistic,fill=niceConfounderLabel))+
  geom_col(position="dodge")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45,hjust=1),text=element_text(size=14),strip.background = element_rect(fill="lightblue"))+
  xlab("")+
  ...
  scale_y_continuous(limits=c(ylimmin_rValues_13mer_nonsep,ylimmax_rValues_13mer_nonsep))

allConfounderMantelResultsPlot_rValues_NOPHYLOMANTEL_13meronly

ggsave(paste0(confounderManteldir,"allMantelResults.allConfounders.AllSpectra.rvaluesOnly.PLUSPHYLODISTFORCOMPARISON.NOPHYLOMANTEL.13meronly.rvalues.pdf"),allConfounderMantelResultsPlot_rValues_NOPHYLOMANTEL_13meronly,height=6.5, width=6)
...
##########  plot R values faceted per central bp:##########

# need to expand ylim to encompass annotations:
ylimmin_rValues_3mer=min(data.frame(all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist[all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist$id %in% c("3-mer spectrum") & all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist$niceLabelForPlotting!="phyloMantel",])$statistic)
ylimmax_rValues_3mer=max(data.frame(all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist[all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist$id %in% c("3-mer spectrum") & all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist$niceLabelForPlotting!="phyloMantel",])$statistic) 
# and add 10%:
ylimmax_rValues_3mer=ylimmax_rValues_3mer+.1*ylimmax_rValues_3mer

allConfounderMantelResultsPlot_SEPBYCENTRALBP_rValues_3merOnly_forms <- ggplot(data.frame(all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist[all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist$id %in% c("3-mer spectrum") & all_confounder_spectrum_mantel_results_SEPARATED_BY_MUTATION_TYPE_PLUSphylodist$niceLabelForPlotting!="phyloMantel",]),aes(x=niceConfounderLabel,y=statistic,fill=niceConfounderLabel))+
  geom_col(position="dodge")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45,hjust=1),text=element_text(size=14))+
  xlab("")+
  ...
  scale_y_continuous(limits=c(ylimmin_rValues_3mer,ylimmax_rValues_3mer),breaks=seq(round(ylimmin_rValues_3mer,1),round(ylimmax_rValues_3mer,1),0.2))

allConfounderMantelResultsPlot_SEPBYCENTRALBP_rValues_3merOnly_forms

ggsave(paste0(confounderManteldir_sepbymutationtype,"allMantelResults.allConfounders.AllSpectra.rvaluesOnly.PLUSPHYLODISTFORCOMPARISON.SEPARATEDBYBP.3meronly.nophylomantel.forms.rvals.pdf"),allConfounderMantelResultsPlot_SEPBYCENTRALBP_rValues_3merOnly_forms,height=4, width=11)
...
# R values showing that phylomantel and regular mantel have same rvalues:
# note this includes both methods! 
...
# separate 1/3 and 5/7mer
# now can plot:
cosine_dissimilarity_plot_SQRTPHYLODIST_13mer <- ggplot(cosineDisimilarityTables_mergedWithPhyloDistance[cosineDisimilarityTables_mergedWithPhyloDistance$id %in% c("1-mer spectrum","3-mer spectrum"),],aes(x=sqrt(cophenetic_distance),y=cosine_dissimilarity))+
  geom_point(size=1)+
  facet_wrap(~id,scales="free")+
  geom_text(data=mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df[mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df$id %in% c("1-mer spectrum","3-mer spectrum"),],aes(x=0.5,y=Inf,vjust=1.1,label=paste0("Pearson's r:  ",signif(statistic,2),"\np-value: ",as.character(scientific(signif,2))," (Mantel Test)")),size=5)+
  ggtitle(paste0("Correlation coefficients from Mantel Test (Pearson's r)\n",unique(mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df[mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df$id %in% c("1-mer spectrum","3-mer spectrum"),]$permCount)," permutations-- SQRT of phylo distance"))+
  theme_bw()+
  theme(strip.background = element_rect(fill="lightblue"),strip.text=element_text(size=16),text=element_text(size=12))+
  xlab("square root of phylogenetic distance (shared branch length)\nbranch lengths represent expected substitutions per site")+
  ylab("Cosine dissimilarty between mutation spectra")

ggsave(paste0(manteldir,"mantelTest.COSINESIMILARITY.SQRTDISTANCE.1-3mer.pdf"),cosine_dissimilarity_plot_SQRTPHYLODIST_13mer,height=4,width=11)
ggsave(paste0(manteldir,"mantelTest.COSINESIMILARITY.SQRTDISTANCE.1-3mer.png"),cosine_dissimilarity_plot_SQRTPHYLODIST_13mer,height=4,width=11)


# now can plot:
cosine_dissimilarity_plot_SQRTPHYLODIST_57mer <- ggplot(cosineDisimilarityTables_mergedWithPhyloDistance[cosineDisimilarityTables_mergedWithPhyloDistance$id %in% c("5-mer spectrum","7-mer spectrum"),],aes(x=sqrt(cophenetic_distance),y=cosine_dissimilarity))+
  geom_point(size=1)+
  facet_wrap(~id,scales="free")+
  geom_text(data=mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df[mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df$id %in% c("5-mer spectrum","7-mer spectrum"),],aes(x=0.5,y=Inf,vjust=1.1,label=paste0("Pearson's r:  ",signif(statistic,2),"\np-value: ",as.character(scientific(signif,2))," (Mantel Test)")),size=5)+
  ggtitle(paste0("Correlation coefficients from Mantel Test (Pearson's r)\n",unique(mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df[mantel_test_results_raxml_SQRTDISTANCE_COSINEDISIMILARITY_df$id %in% c("5-mer spectrum","7-mer spectrum"),]$permCount)," permutations-- SQRT of phylo distance"))+
  theme_bw()+
  theme(strip.background = element_rect(fill="lightblue"),strip.text=element_text(size=16),text=element_text(size=12))+
  xlab("square root of phylogenetic distance (shared branch length)\nbranch lengths represent expected substitutions per site")+
  ylab("Cosine dissimilarty between mutation spectra")

ggsave(paste0(manteldir,"mantelTest.COSINESIMILARITY.SQRTDISTANCE.5-7mer.pdf"),cosine_dissimilarity_plot_SQRTPHYLODIST_57mer,height=4,width=11)
ggsave(paste0(manteldir,"mantelTest.COSINESIMILARITY.SQRTDISTANCE.5-7mer.png"),cosine_dissimilarity_plot_SQRTPHYLODIST_57mer,height=4,width=11)
...
cosineSimilarityViolinPlot2 <- ggplot(cosineDisimilarityTables_mergedWithPhyloDistance[cosineDisimilarityTables_mergedWithPhyloDistance$id %in% c("1-mer spectrum","3-mer spectrum"),],aes(x=id,y=cosine_similarity))+
  ...
  xlab("")+
  scale_y_continuous(breaks=seq(0.90,1,by=0.01))
...
cosineSimilarityViolinPlot3 <- ggplot(cosineDisimilarityTables_mergedWithPhyloDistance[cosineDisimilarityTables_mergedWithPhyloDistance$id %in% c("5-mer spectrum","7-mer spectrum"),],aes(x=id,y=cosine_similarity))+
  ...
  theme(text=element_text(size=14))+
  xlab("")+
  scale_y_continuous(breaks=seq(0.60,1,by=0.05))
