...
chrspecificvcf=$wd/vcfs_with_ancestral_alleles_KimuraModel_assignedUsingProbs/perChromosome/${chromosome}.AncAllelesFromExtSFSAssignedUsingProbs.KimuraModel.BiallelicSNPsOnly.BUGFIX.AllMouse.vcf_90_recalibrated_snps_raw_indels_reheader_PopSorted.PASS.vcf.gz 

refFasta=$refdir/${chromosome}.fa
prefix=${chromosome}_mouseAncestral_extSFS_assignedUsingProbs.BUGFIX ### CHANGE THIS FOR DIFFERENT RUNS BASED ON OTHER CRITERIA
outdir=$projectdir/analyses/ancestralReferenceFastas/${flagForThisRun}_KimuraModel_AncAlleleAssignedUsingProbs
mkdir -p $outdir
