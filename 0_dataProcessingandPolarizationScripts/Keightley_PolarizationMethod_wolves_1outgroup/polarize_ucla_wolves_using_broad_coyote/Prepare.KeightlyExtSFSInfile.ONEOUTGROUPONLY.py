...
if sum([ x not in focalSamples+outgroup1Samples for x in samples])>0:
...
for line0 in inVCF:
    ...
    # okay time to write out: (adding major and minor) (removing outgroup2)
    output = [str(mychr), str(mypos), str(myref),str(myalt),str(MAJOR),str(MINOR),",".join(str(count) for count in focal_counts), ",".join(str(count) for count in outgroup1_counts)]
    #print("\t".join(output))
